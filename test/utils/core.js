...
module.exports = function utilsCoreTests() {
  ...
  describe("validateType", () => {
    ...
    it("should return the type", () => {
      ...
      expect(
        validateType("abc", "val", ["number", "array", "string"])
      ).to.equal("string");
      expect(
        validateType(true, "val", ["number", "array", "boolean"])
      ).to.equal("boolean");
      expect(
        validateType(false, "val", ["number", "array", "boolean"])
      ).to.equal("boolean");
      expect(validateType(null, "val", ["number", "object", "null"])).to.equal(
        "null"
      );
      expect(
        validateType(undefined, "val", ["number", "object", "undefined"])
      ).to.equal("undefined");
      expect(validateType(NaN, "val", ["number", "object", "NaN"])).to.equal(
        "NaN"
      );
      expect(
        validateType([], "val", ["number", "array", "undefined"])
      ).to.equal("array");
      expect(
        validateType({}, "val", ["number", "object", "undefined"])
      ).to.equal("object");
    });
    ...
    it("should throw if none of the validators match", () => {
      ...
      expect(() =>
        validateType(1, "val", ["null", "object", "array"])
      ).to.throw(errors.BadRequest);
      expect(() =>
        validateType("abc", "val", ["number", "object", "undefined"])
      ).to.throw(errors.BadRequest);
      expect(() =>
        validateType(true, "val", ["number", "object", "array"])
      ).to.throw(errors.BadRequest);
      expect(() =>
        validateType(null, "val", ["number", "object", "string"])
      ).to.throw(errors.BadRequest);
      expect(() =>
        validateType([], "val", ["number", "object", "null"])
      ).to.throw(errors.BadRequest);
    });
  });
  ...
  describe("removeProps", () => {
    ...
    it("should work if some properties are not defined on the object", () => {
      expect(removeProps(object, "_meta", "not_there")).to.deep.equal({
        _id: 12,
        age: 13,
      });
    });
